          ...
          <div style={{maxWidth:1200,margin:"0 auto",padding:"32px 16px"}}>
            ...
            <div style={{display:"flex",gap:3,marginBottom:32,flexWrap:"wrap"}}>
              {[["✂️",tailorServices.length,"SERVICES LISTED"],["📍",[...new Set(tailorServices.map(s=>s.location).filter(Boolean))].length,"LOCATIONS"],["⭐","10%","PLATFORM FEE"],["🔒","100%","SECURE PAYMENTS"]].map(([e,n,l])=>(
                <div key={l} style={{flex:"1 1 120px",background:"#fafafa",border:"2px solid #111",padding:"16px 20px",display:"flex",alignItems:"center",gap:12}}>
                  ...
                  <div>
                    <p style={{fontFamily:"'Barlow Condensed',sans-serif",fontSize:22,fontWeight:900,color:"#00E5CC",lineHeight:1}}>{n}</p>
            ...
            {user&&myTailorServices.length>0&&(
              <div style={{marginBottom:40}}>
                <div style={{fontFamily:"'Barlow Condensed',sans-serif",fontSize:11,fontWeight:900,letterSpacing:3,color:"#00E5CC",borderLeft:"4px solid #00E5CC",paddingLeft:12,marginBottom:16}}>MY LISTED SERVICES</div>
                <div style={{display:"flex",flexDirection:"column",gap:8}}>
                  {myTailorServices.map(s=>(
                    <div key={s.id} style={{border:"2px solid #00E5CC",padding:"16px 20px",display:"flex",alignItems:"center",justifyContent:"space-between",gap:16,flexWrap:"wrap",background:"#fffbf5"}}>
                      <div style={{display:"flex",alignItems:"center",gap:14}}>
                        <div style={{width:48,height:48,background:"#00E5CC",display:"flex",alignItems:"center",justifyContent:"center",fontSize:24,flexShrink:0}}>✂️</div>
                        <div>
                          <p style={{fontFamily:"'Barlow Condensed',sans-serif",fontSize:16,fontWeight:900,marginBottom:2}}>{s.title}</p>
                          <p style={{fontFamily:"'Barlow Condensed',sans-serif",fontSize:11,color:"#00E5CC",letterSpacing:1}}>{s.service_type?.toUpperCase()} · FROM {currencySymbol(profile?.currency)}{s.price_from}{s.turnaround_days?` · ${s.turnaround_days} DAYS`:""}</p>
                        </div>
                      </div>
                      <div style={{display:"flex",gap:8}}>
                        <button className="hbtn" style={{...S.hBtn,background:"#00E5CC",border:"none",fontSize:11,padding:"8px 14px"}} onClick={()=>{setEditingService(s);setTailorServiceForm({title:s.title,description:s.description||"",service_type:s.service_type||"Alterations",price_from:s.price_from||"",price_to:s.price_to||"",turnaround_days:s.turnaround_days||"",location:s.location||"",images:[],imagePreviews:s.images||[]});setShowTailorForm(true);}}>EDIT</button>
            ...
            {tailorServices.length===0?(
              <div style={{textAlign:"center",padding:"80px 20px",border:"3px dashed #e0e0e0"}}>
                ...
                {user&&<button className="hbtn" style={{...S.hBtn,background:"#00E5CC",border:"none",padding:"14px 32px",fontSize:14,letterSpacing:2}} onClick={()=>setShowTailorForm(true)}>LIST MY SERVICE →</button>}
              </div>
            ):(
              ...
              <div style={S.modalBox} onClick={e=>e.stopPropagation()}>
                ...
                <div style={{background:"#fafafa",border:"2px solid #f0f0f0",padding:"16px",marginBottom:20,display:"flex",gap:14,alignItems:"flex-start"}}>
                  ...
                  <div>
                    ...
                    <p style={{fontFamily:"'Barlow Condensed',sans-serif",fontSize:12,color:"#00E5CC",letterSpacing:1}}>{selectedService.service_type?.toUpperCase()} · FROM {currencySymbol(profile?.currency||"GBP")}{selectedService.price_from}</p>
                ...
                <div style={{marginBottom:20,padding:"14px 16px",background:"#fff8f0",border:"1.5px solid #00E5CC55"}}>
                  <p style={{fontFamily:"'Barlow Condensed',sans-serif",fontSize:11,fontWeight:800,letterSpacing:2,color:"#00E5CC",marginBottom:8}}>HOW IT WORKS</p>
                  <div style={{display:"flex",flexDirection:"column",gap:6}}>
                    {["Send booking request with your notes","Tailor responds via Stitch'd messages","Agree on final price and details","Pay securely through Stitch'd (10% fee)","Tailor completes your order 🩷"].map((s,i)=>(
                      <div key={i} style={{display:"flex",gap:10,alignItems:"flex-start"}}>
                        <span style={{fontFamily:"'Barlow Condensed',sans-serif",fontSize:12,fontWeight:900,color:"#00E5CC",flexShrink:0}}>{i+1}.</span>
                ...
                <button className="hbtn" style={{...S.hBtn,background:"#00E5CC",border:"none",width:"100%",padding:"16px",fontSize:15,letterSpacing:3,marginTop:16}} onClick={()=>bookTailor(selectedService)}>
          ...
          {showTailorForm&&(
            <div style={S.modalOverlay} onClick={()=>setShowTailorForm(false)}>
              <div style={{...S.modalBox,maxWidth:580}} onClick={e=>e.stopPropagation()}>
                ...
                {/* Profile reminder */}
                {!profile?.avatar_url&&(
                  <div style={{background:"#fff8f0",border:"1.5px solid #00E5CC55",padding:"12px 16px",marginBottom:20,display:"flex",gap:10,alignItems:"center"}}>
                    <span style={{fontSize:20}}>💡</span>
                    <div>
                      <p style={{fontFamily:"'Barlow Condensed',sans-serif",fontSize:12,fontWeight:800,letterSpacing:1,color:"#00E5CC",marginBottom:2}}>ADD A PROFILE PHOTO</p>
                ...
                <button className="hbtn" style={{...S.hBtn,background:"#00E5CC",border:"none",width:"100%",padding:"16px",fontSize:15,letterSpacing:3,marginTop:20,opacity:(!tailorServiceForm.title||!tailorServiceForm.price_from)?0.4:1}} onClick={saveTailorService} disabled={!tailorServiceForm.title||!tailorServiceForm.price_from}>
